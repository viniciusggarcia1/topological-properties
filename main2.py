...
import numpy as np
import qsh  # Supondo que você tenha um módulo 'qsh' que contém as funções usadas no código
import time
...
for i, kx in enumerate(tqdm(kx_range, desc='Calculating Berry phase')):
...
for i, kx in enumerate(tqdm(kx_range, desc='Calculating energies')):
...
for i, ky in enumerate(tqdm(ky_range, desc='Calculating Berry phase for fixed kx')):
...
# Calcular a fase de Berry para kx fixo e ky variando
for i, kx in enumerate(tqdm(kx_range, desc='Calculating Berry phase for fixed ky')):
...
for kx in tqdm(range(0, len(kx_range), 1), desc='Calculating projections'):
    ky = kx
...
#Chamaremos todas as funções de plot para salvar os gráficos e mostraremos o progresso
with tqdm(total=100) as prog:
    ...
    prog.update(20)
    tqdm(qsh.graph_ky_fixed(kx_range, phases_fixed_ky, fixed_ky))
    prog.update(20)
